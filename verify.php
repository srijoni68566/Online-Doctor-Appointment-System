...
<head>
  ...
  <link href="css/style.css" rel="stylesheet" type="text/css">
	...
	<style>
  .form-container{
	boder:0px solid #fff;padding:50px 60px;margin-top:6vh;
	margin-right:400px;
	margin-left:400px;
...
<body>
	...
	<span>
     <h1><b align="center">Doctor Verification</b></h1>
...
<?php
  ...
  include('dbcon.php');       /*connection with database*/
     ...
     if(isset($_POST['verify'])) //after log in button press
  {
	  $name = $_POST['uname']; //input deya info gulo variable e store
	  $password =$_POST['pass'];
	  
	  $qry="SELECT * FROM temp_doc WHERE name='$name' AND password='$password'"; //database er info er sathe matching
	  ...
	  {
		  ?>
		  <script>alert('Your profile not added/wrong password ');
	  ...
	  $SQL ="INSERT INTO doctor_details(avail,name,speciality,start_time,finish_time,doc_contact,fee,password,firstname,lastname,degree,limitt)VALUES
	       ('YES','$name','$speciality','$stime','$ftime','$contact','$fee','$password','$firstname','$lastname','$degree','$limitt');";
		    $run1=mysqli_query($con,$SQL);
			 if($run1==true){
				 $SQL2="DELETE FROM temp_doc WHERE name='$name' AND password='$password';";
				 $run2=mysqli_query($con,$SQL2);
				 			 if($run2==true){
		  ?>
		  <script>
		      alert('Doctor verification is successful.Thank you!!!');
			 window.open('doclogin.php','_self');
		  </script>
	 <?php
